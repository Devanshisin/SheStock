    ...
    with col2:
        if st.button("🔍 Predict Recommendation", use_container_width=True):
            if model is not None:
                ...
                proba = model.predict_proba(input_df)[0][1] if hasattr(model, "predict_proba") else 0.75  # Default confidence if no proba
                ...
                similar_stocks = find_similar_stocks(company_data, real_stocks_df)
                if len(similar_stocks) > 0:
                    st.markdown("### 🏢 Companies Matching Your Criteria")
                    for _, row in similar_stocks.head(5).iterrows():
                        st.markdown(f"""
                        <div class="stock-card">
                            <h4>{row['Company']} ({row['Ticker']})</h4>
                            <strong>Sector:</strong> {row['Sector']} | 
