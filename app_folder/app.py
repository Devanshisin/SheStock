...
st.markdown("Empowering women-led sustainable investment decisions.")
...
with col1:
    female_ceo = st.selectbox("👩‍💼 Female CEO?", [0, 1], index=1, format_func=lambda x: "Yes" if x else "No")
    female_board = st.slider("🧑‍🤝‍🧑 Female Board Members", 0, 6, 2)
...
company_data = {
    ...
    "Female_Board_Members": female_board,
    "ESG_Score": esg_score,
    **{f"Sector_{key}": int(value) for key, value in sectors.items()}
}

expected_cols = [
    "Female_CEO", "Female_Board_Members", "ESG_Score",
    "Sector_Energy", "Sector_FMCG", "Sector_Finance", "Sector_Healthcare",
    "Sector_IT", "Sector_Insurance", "Sector_Mining", "Sector_Pharma", "Sector_Retail"
]
...
if st.button("🔍 Predict Recommendation"):
    ...
    if pred:
        confidence_class = "🟢 High" if proba > 0.8 else "🟡 Medium" if proba > 0.6 else "🔴 Low"
        st.success(f"✅ RECOMMENDED — Confidence: {proba*100:.1f}% ({confidence_class})")
    else:
        confidence_class = "🟢 High" if (1-proba) > 0.8 else "🟡 Medium" if (1-proba) > 0.6 else "🔴 Low"
        st.error(f"🚫 NOT RECOMMENDED — Confidence: {(1 - proba) * 100:.1f}% ({confidence_class})")

# Prediction history
with st.expander("📈 Prediction History"):
    if st.session_state.prediction_history:
        for i, p in enumerate(reversed(st.session_state.prediction_history[-10:])):
            st.markdown(f"**{len(st.session_state.prediction_history) - i}.** {p['timestamp'].strftime('%Y-%m-%d %H:%M')} — {'✅ Recommended' if p['prediction'] else '🚫 Not Recommended'} ({p['confidence']*100:.1f}%)")
            st.json(p['data'])
        if st.button("🗑️ Clear History"):
            st.session_state.prediction_history = []
            st.rerun()
...
with st.expander("📚 Glossary"):
    st.markdown("""
    - **ESG Score**: Environmental, Social, and Governance score (0.0 to 1.0)
    - **Female CEO / Board Members**: Women in leadership positions
    - **Sectors**: Industry classification of the company
    - **Recommendation**: Model prediction to invest or not
    """)
